...
@if($imagenes->count() == 0)
	...
	{{ Form::open(array('url' => 'admin/saveImg/' .$id_propiedad , 'files' => true)) }}
		...
		<table class="table table-striped table-hover table-condensed panel-primary">
			...
			<tbody >
				@foreach($imagenes as $value)
					<?php 
						$count = DB::table('propiedades_img')->where('id_propiedad',$value->id_propiedad)->count();
					...
					<tr>
						...
						<td>
							...
							<img class="watermark" src="{{ asset('upload/'. $value->ruta .'') }}" class="img-rounded" width="70%" >
						</td>
						<td>
